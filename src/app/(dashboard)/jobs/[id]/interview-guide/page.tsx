...
export default function InterviewGuidePage({ params }: Props) {
  ...
  const handleApprove = async () => {
    ...
    const res = await fetch(`/api/workflows/${id}/outputs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "interview_guide", content, status: "approved" }),
    });
  ...
  const handleUnapprove = async () => {
    await fetch(`/api/workflows/${id}/outputs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "interview_guide", content, status: "draft" }),
    });
    setStatus("draft");
  };
  ...
  const downloadMarkdown = () => {
    ...
    const md = `# Interview Guide\n${ro} · ${co}\n\n${content}`;
    const blob = new Blob([md], { type: "text/markdown" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `${co.replace(/\s+/g, "_")}_Interview_Guide.md`; a.click();
