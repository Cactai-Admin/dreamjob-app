...
export default function InterviewGuidePage({ params }: Props) {
  ...
  const [docState, setDocState] = useState<NativeDocument>(parseNativeDocument("interview_guide"));
  ...
  const saveContent = useCallback(async (text: string) => {
    const payloadDoc = setPrimarySectionText(parseNativeDocument("interview_guide"), text);
    const res = await fetch(`/api/workflows/${id}/outputs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "interview_guide", content: serializeNativeDocument(payloadDoc) }),
    });
  ...
  const fetchWorkflow = useCallback(async () => {
    try {
      ...
      const syncDoc = (wf.autosave_data as { google_docs_sync?: { docs?: { interview_guide?: { documentUrl?: string; syncState?: string; error?: string } } } } | null)
        ?.google_docs_sync?.docs?.interview_guide;
      if (syncDoc) {
      ...
      if (out) {
        setOutput(out);
        setDocState(parseNativeDocument("interview_guide", out));
        setGenerating(false);
  ...
  const handleDeleteGuide = useCallback(async () => {
    ...
    setOutput(undefined);
    setDocState(parseNativeDocument("interview_guide"));
    setIsDirty(false);
